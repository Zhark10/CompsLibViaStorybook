...
class ButtonComponent extends React.Component<Props> {
  ...
  render() {
    const {
      ...
      disabled, shadow, loading, backgroundColor
    } = this.props;
    const buttonColor = (loading || disabled) ? {backgroundColor: '#cccccc', color: '#666666'} : {backgroundColor, color};
    ...
    return (
      <TouchableOpacity
        ...
        style={[
          ...
          shadowStyles]}
      >
      {loading ? <ActivityIndicator size="small" color="#666666" /> :
